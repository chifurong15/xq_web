...
app.controller(
	...
	[
		...
		function menuAuthorityController($localStorage, $scope, $location, $log,
										 $q, $rootScope, $window, routeService, $http, moduleService) {
			...
			$scope.init = function (functionType) {
			...
			function zTreeOnClick(event, treeId, treeNode) {
				...
				$http({
					...
					params: {roleId: treeNode.id, functionType: $scope.functionType},
				}).success(
					function (res) {
						console.log(res)
						var setting2 = {
...
//										chkboxType: { "Y": "s", "N": "s" }
			...
			$scope.changess = function () {
				...
				$http({
					...
					params: {powers: arr, roleId: data, functionType: $scope.functionType}
				}).success(
					function (res) {
						alert("授权成功")
						$scope.reloadRoute();
