    ...
    angular.module("app").controller(
        ...
        [
            ...
            function imReachEdit($localStorage, $scope, $location, $log, $q, $rootScope, globalParam, $window, routeService, moduleService, $http,
                ...
                $scope.reachDetailData = $localStorage.reachEditData;
                $scope.reachDetailData.waterGrade = String($scope.reachDetailData.waterGrade);
                $scope.reachDetailData.silty = String($scope.reachDetailData.silty);
                ...
                //增加管理人员
                $scope.reachadmins = [];
                angular.forEach($scope.reachDetailData.userList,function(each){
                    var isassess = each.isAssess == 'Y' ? true : false;
                    $scope.reachadmins.push({id: each.reachChairManId, chairmanName: each.userName, chairmanid: each.userId, chairmanlevel: String(each.grade), chairmanRole: each.chairmanRole, administrationCoast: each.shore, isAssess:isassess})
                })
                $scope.addReachadmin = function($index){
                    $scope.reachadmins.push({chairmanName: "",chairmanid:"",chairmanlevel:"",chairmanRole:"",administrationCoast:"",isAssess:"",id:""});
                }
                ...
                $scope.add = function() {
                    ...
                    var data = {
                        ...
                        alias: param.alias,
                        waterGrade: param.waterGrade,
                        silty: param.silty,
                        regionName: param.regionName,
                        regionCode: param.regionCode,
                        waterName: param.waterName,
                        waterCode: param.waterCode,
                        grade: param.grade,
                        startPoint: param.startPoint,
                        endPoint: param.endPoint,
                        whether: param.whether,
                        isVirtual: param.isVirtual,
                        length: param.length,
                        width: param.width,
                        throughArea: param.throughArea,
                        parentsCode: param.parentsCode,
                        shore: param.shore,
                        pName: param.pName,
                    ...
                    if(!param.reachName || !jsname.test(param.reachName) == null) {
                        ...
                        var flag = 1;
                        var reachadmins =  [].concat(JSON.parse(JSON.stringify($scope.reachadmins)));
                        angular.forEach(reachadmins, function (each){
                            if(each.chairmanid == '') {
                                layer.alert("请完善履职人员", {
                                    skin: 'my-skin',
                                    closeBtn: 1,
                        ...
                        data.reachChairmanJson = param.reachChairmanJson;
                    }
                    $http({
                        method: "post",
