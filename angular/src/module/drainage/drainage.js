		...
		app.controller(
			'drainageCtrl', [
				...
				function drainageCtrl($localStorage, $scope,$location, $log, $q, $rootScope, globalParam,$window,routeService, moduleService, $http) {
					 ...
					 */
					$scope.$watch('basinName',  function(n, o) {
				        if (n) { 
				        	return; 
				        }
				        reGetProducts();
					...
					$scope.drainageDetail = function(id) {
						$http({
							...
							if(res.resCode == 1){
								$localStorage.drainageDetailData = res;
								console.log($localStorage.drainageDetailData)
					...
					$scope.drainageEdit = function(id) {
						$http({
							...
							if(res.resCode == 1){
								$localStorage.drainageEditData = res;
