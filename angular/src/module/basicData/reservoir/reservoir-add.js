...
(function(window, angular) {
	'use strict';
	angular.module("app").controller('reservoirAddCtrl', ['$localStorage','$scope','moduleService','routeService','$http','MapUtil','OneMapqueryAdminregionFromMysql',
		function reservoirAddCtrl($localStorage, $scope,moduleService,routeService, $http, MapUtil,OneMapqueryAdminregionFromMysql) {
			$scope.reservoir = {};
            //判断名称是否重复
            $scope.isRepeat = function(){
            	if(!$scope.reservoir.reservoirName){
                    return;
                }
                $http({
                    method:'get',
			...
			$scope.submit = function() {
                ...
                $http({
					method: "post",
					url: moduleService.getServiceUrl() +reservoirUrl + "/add",
					data: {
