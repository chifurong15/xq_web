	...
	angular.module("app")
		.controller(
			'waterAddCtrl', [
				...
				'moduleService',
				'$http',
				function waterAddCtrl($localStorage, $scope, $q, $rootScope, $window,routeService,moduleService, $http) {
					$scope.water = {};

					//判断名称是否重复
                    $scope.isRepeat = function(){
                    	if(!$scope.water.waterName){
                    		return;
                    	}
                        $http({
                            method:'get',
					...
					$scope.waterAdd = function() {
						...
						$http({
							method: "post",
							url: moduleService.getServiceUrl() + waterUrl + "/add",
							data: {
