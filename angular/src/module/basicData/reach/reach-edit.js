...
(function(window, angular) {
    ...
    angular.module("app").controller(
        'imReachEdit',['$localStorage','$scope','$rootScope','routeService','moduleService','$http','MapUtil','OneMapqueryAdminregionFromMysql',
        function imReachEdit($localStorage, $scope, $rootScope, routeService,moduleService, $http,MapUtil,OneMapqueryAdminregionFromMysql) {
            if(!$localStorage.reachEditData){
                layer.msg('获取数据失败,请重试')
            }
            $scope.reach = $localStorage.reachEditData;
            /*所属区域树模态框*/
            ...
            //判断名称是否重复
            var oldName = $scope.reach.reachName;
            $scope.isRepeat = function() {
                if($scope.reach.reachName == oldName){
                    return;
                }else if(!$scope.reach.reachName){
                    return;
                }
                $http({
                    method:'get',
            ...
            $scope.convertToInt = function (id) { //ng-option绑定数值型数据无效
                return parseInt(id);
            };
        }]);
})(window, angular);